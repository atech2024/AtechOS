...
export default function AttendancePage() {
  ...
  async function loadRoster() {
    ...
    const { data: enrollments, error: ee } = await supabase.from('enrollments').select('student_id,students(id,first_name,last_name)').eq('class_id', classId)
  ...
  async function save(student: Student, status: string) {
    ...
    const { data, error } = await supabase.rpc('record_attendance', {
      ...
      p_check_out_at: current?.check_out_at || null,
      p_late_minutes: status === 'late' ? (current?.late_minutes || 0) : 0,
  ...
  return <main className="min-h-screen bg-slate-50 p-6 md:p-10"><div className="mx-auto max-w-7xl">
    <header><p className="text-sm font-semibold text-blue-600">AtechOS</p><h1 className="mt-1 text-3xl font-bold text-slate-900">Attendance</h1><p className="mt-1 text-slate-500">Record daily presence, lateness and early departure.</p></header>
    ...
    <section className="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-200 p-5"><h2 className="font-semibold">Daily roster</h2><p className="mt-1 text-sm text-slate-500">{students.length} student{students.length === 1 ? '' : 's'}</p></div>{loading ? <p className="p-5 text-slate-500">Loading...</p> : students.length === 0 ? <p className="p-5 text-slate-500">No students are enrolled in this class yet.</p> : <div className="divide-y divide-slate-100">{students.map(student => { const r = records[student.id]; return <div key={student.id} className="flex flex-col gap-3 p-5 md:flex-row md:items-center md:justify-between"><div><p className="font-semibold text-slate-900">{student.first_name} {student.last_name}</p><p className="text-xs text-slate-500">{r?.check_in_at ? `Check-in ${new Date(r.check_in_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}` : 'No check-in'}</p></div><div className="flex flex-wrap gap-2">{statuses.map(status => <button key={status} disabled={saving === student.id} onClick={() => save(student, status)} className={`rounded-lg px-3 py-2 text-xs font-semibold ${r?.status === status ? 'bg-blue-600 text-white' : 'border border-slate-300 bg-white text-slate-700'}`}>{status.replace('_', ' ')}</button>)}</div></div>})}</div>}</section>
  </div></main>
}
