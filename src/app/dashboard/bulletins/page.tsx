...
type Period = { id: string; name: string; code: string; weight: number; start_date: string; end_date: string }
...
export default function BulletinsPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [schoolResult, classResult, periodResult] = await Promise.all([
      ...
      if (schoolResult.error || classResult.error || periodResult.error) setError((schoolResult.error || classResult.error || periodResult.error)?.message || 'Could not load bulletin data.')
      setSchool(schoolResult.data as School | null)
      setClasses((classResult.data || []) as ClassItem[])
      setPeriods((periodResult.data || []).filter((p: Period) => p.is_active) as Period[])
      setLoading(false)
    }
    load()
  }, [])
  ...
  const rows = useMemo(() => subjects.map(subject => {
    const periodRows = periods.map(period => {
      ...
      return { period, average: weightedMax ? (weightedScore / weightedMax) * 100 : null }
    })
  ...
  return <main className="min-h-screen bg-slate-50 p-6 md:p-10"><div className="mx-auto max-w-6xl">
    <header className="mb-8"><p className="text-sm font-semibold text-blue-600">AtechOS</p><h1 className="mt-1 text-3xl font-bold text-slate-900">Bulletins</h1><p className="mt-1 text-slate-500">Generate and print a student bulletin. The browser print dialog can save it as PDF.</p></header>
    ...
    <section className="mb-6 grid gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm md:grid-cols-2 print:hidden">
      <label className="text-sm font-medium">Class<select value={classId} onChange={e => setClassId(e.target.value)} className="mt-2 w-full rounded-xl border border-slate-300 px-3 py-3"><option value="">Select class...</option>{classes.map(c => <option key={c.id} value={c.id}>{c.name}{c.grade_levels?.short_name ? ` · ${c.grade_levels.short_name}` : ''}{c.academic_years?.name ? ` · ${c.academic_years.name}` : ''}</option>)}</select></label>
    ...
    {student && <article className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm print:border-0 print:p-0 print:shadow-none">
      <div className="flex items-start justify-between gap-6 border-b border-slate-200 pb-6"><div>{school?.logo_url && <img src={school.logo_url} alt="School logo" className="mb-3 h-16 w-16 object-contain"/>}<h2 className="text-2xl font-bold text-slate-900">{school?.name || 'School'}</h2><p className="text-sm text-slate-500">{school?.code}{school?.address ? ` · ${school.address}` : ''}</p><p className="text-sm text-slate-500">{school?.phone || ''}{school?.email ? ` · ${school.email}` : ''}</p></div><div className="text-right"><h3 className="text-xl font-bold text-slate-900">Student Bulletin</h3><p className="mt-1 text-sm text-slate-500">{selectedClass?.name || ''}</p><p className="text-sm text-slate-500">{student.first_name} {student.last_name}</p><p className="text-sm text-slate-500">{student.student_code || '—'}</p></div></div>
      <div className="mt-6 overflow-x-auto"><table className="w-full border-collapse text-sm"><thead><tr className="border-b-2 border-slate-300"><th className="px-3 py-3 text-left">Subject</th>{periods.map(p => <th key={p.id} className="px-3 py-3 text-right">{p.name}</th>)}<th className="px-3 py-3 text-right">Final</th></tr></thead><tbody>{rows.map(r => <tr key={r.subject.id} className="border-b border-slate-200"><td className="px-3 py-3 font-medium">{r.subject.name}</td>{r.periodRows.map(pr => <td key={pr.period.id} className="px-3 py-3 text-right">{pr.average === null ? '—' : `${pr.average.toFixed(2)}%`}</td>)}<td className="px-3 py-3 text-right font-semibold">{r.finalAverage === null ? '—' : `${r.finalAverage.toFixed(2)}%`}</td></tr>)}</tbody></table></div>
      <div className="mt-8 flex justify-end"><div className="rounded-xl border border-slate-200 px-6 py-4 text-right"><p className="text-sm text-slate-500">Overall average</p><p className="text-2xl font-bold text-slate-900">{overall === null ? '—' : `${overall.toFixed(2)}%`}</p></div></div>
      <div className="mt-12 grid grid-cols-2 gap-12 text-center text-sm"><div className="border-t border-slate-400 pt-2">Parent / Guardian</div><div className="border-t border-slate-400 pt-2">School Administration</div></div>
    </article>}
