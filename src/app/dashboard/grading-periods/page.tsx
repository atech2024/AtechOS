...
export default function GradingPeriodsPage() {
 ...
 return <main className="mx-auto max-w-5xl p-6"><h1 className="text-3xl font-bold"><T text="Grading periods"/></h1><p className="my-3">Choose the calendar and sections that use each period. Your school sets the dates.</p>{error && <p role="alert" className="my-3 text-red-700">{error}</p>}{message && <p role="status">{message}</p>}<form onSubmit={save} className="my-6 grid gap-4 rounded-xl border bg-white p-5 md:grid-cols-2"><label><T text="Academic year"/><select required value={year} onChange={e=>setYear(e.target.value)} className="block w-full rounded border p-3"><option value=""><T text="Select year"/></option>{years.map(y=><option key={y.id} value={y.id}>{y.name}</option>)}</select></label><label><T text="Period"/><select value={preset} onChange={e=>setPreset(e.target.value)} className="block w-full rounded border p-3">{periodPresets.map(([code,name])=><option key={code} value={code}>{name}</option>)}<option value="custom">Other - add a period</option></select></label>{preset==='custom' && <><label><T text="Name"/><input name="name" required className="block w-full rounded border p-3" /></label><label><T text="Code"/><input name="code" required className="block w-full rounded border p-3" /></label></>}<label><T text="Start date"/><input name="start" type="date" required className="block w-full rounded border p-3" /></label><label><T text="End date"/><input name="end" type="date" required className="block w-full rounded border p-3" /></label><fieldset className="md:col-span-2"><legend><T text="Sections using this period"/></legend><div className="my-3 flex flex-wrap gap-4">{schoolSections.map(s=><label key={s.code}><input type="checkbox" checked={sections.includes(s.code)} onChange={e=>setSections(v=>e.target.checked?[...v,s.code]:v.filter(x=>x!==s.code))} /> {s.name}</label>)}</div></fieldset><button disabled={busy || !year || !sections.length} className="rounded bg-blue-600 p-3 text-white disabled:opacity-50">{busy?'Saving...':'Activate period'}</button></form><p className="text-sm text-slate-600">To change dates or sections, select the same period and year above and save again.</p><div className="my-5 space-y-3">{periods.filter(p=>!year || !p.academic_year_id || p.academic_year_id===year).map(p=><article key={p.id} className="rounded-xl border bg-white p-4"><h2 className="font-semibold">{p.name} - {p.is_active?'Active':'Inactive'}</h2><p>{p.start_date} / {p.end_date}</p><p>{p.sections.map(code=>schoolSections.find(s=>s.code===code)?.name).join(', ')}</p>{!p.academic_year_id && <p className="text-sm">Legacy period - no specific academic year.</p>}<button disabled={busy || !year} onClick={()=>toggle(p)} className="mt-2 rounded border p-2">{p.is_active?'Deactivate':'Activate'}</button></article>)}</div></main>
}
