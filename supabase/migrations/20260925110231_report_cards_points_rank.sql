-- One server calculation shared by student, family and administration.
create function private.class_period_report(p_class uuid,p_period uuid)
returns table(student_id uuid,subjects jsonb,earned numeric,possible numeric,complete boolean,average numeric,rank_value bigint,class_size bigint,rank_ready boolean)
language sql stable security definer set search_path='' as $$
 with roster as (
 select e.student_id from public.enrollments e where e.class_id=p_class
 union select g.student_id from public.grades g where g.class_id=p_class and g.grading_period_id=p_period
 ), required as (
 select cs.subject_id,su.name from public.class_subjects cs join public.subjects su on su.id=cs.subject_id where cs.class_id=p_class and su.active
 ), marks as (
 select r.student_id,q.subject_id,q.name,
 count(g.id)>0 and coalesce(bool_and(g.published),false) as ready,
 sum(g.score*coalesce(g.assessment_weight,100)/100) filter(where g.published) as earned,
 sum(g.max_score*coalesce(g.assessment_weight,100)/100) filter(where g.published) as possible,
 coalesce(jsonb_agg(jsonb_build_object('assessment',g.assessment_name,'score',g.score,'max_score',g.max_score,'weight',g.assessment_weight) order by g.graded_at,g.id) filter(where g.published),'[]') as assessments
 from roster r cross join required q left join public.grades g on g.student_id=r.student_id and g.subject_id=q.subject_id and g.class_id=p_class and g.grading_period_id=p_period
 group by r.student_id,q.subject_id,q.name
 ), totals as (
 select r.student_id,coalesce(jsonb_agg(jsonb_build_object('id',m.subject_id,'name',m.name,'ready',m.ready,'note',case when m.ready then m.earned/nullif(m.possible,0)*10 end,'earned',case when m.ready then m.earned end,'possible',case when m.ready then m.possible end,'assessments',m.assessments) order by m.name) filter(where m.subject_id is not null),'[]') subjects,
 sum(m.earned) earned,sum(m.possible) possible,count(m.subject_id)>0 and coalesce(bool_and(m.ready),false) complete
 from roster r left join marks m on m.student_id=r.student_id group by r.student_id
 ), averages as (
 select t.*,case when t.complete then t.earned/nullif(t.possible,0)*10 end average from totals t
 ) select a.student_id,a.subjects,case when a.complete then a.earned end,case when a.complete then a.possible end,a.complete,a.average,
 case when a.complete then rank() over(order by a.average desc nulls last) end,
 count(*) over(),bool_and(a.complete) over() from averages a;
$$;
revoke all on function private.class_period_report(uuid,uuid) from public,anon,authenticated;
...
declare s public.students;begin
 select * into s from public.students where id=p_student;if s.id is null then return null;end if;
 return jsonb_build_object('school',(select jsonb_build_object('name',name,'code',code,'address',address,'phone',phone) from public.schools where id=s.school_id),
 'student',jsonb_build_object('id',s.id,'first_name',s.first_name,'last_name',s.last_name,'atechos_id',s.atechos_id),
 'cards',coalesce((select jsonb_agg(jsonb_build_object('class_id',c.id,'class_name',c.name,'year',y.name,'period_id',p.id,'period',p.name,'start_date',p.start_date,'end_date',p.end_date,'exam_start',d.exam_start,'exam_end',d.exam_end,'subjects',r.subjects,'earned',r.earned,'possible',r.possible,'complete',r.complete,'average',r.average,'position',case when r.rank_ready then r.rank_value end,'class_size',r.class_size,'rank_ready',r.rank_ready,'status',case when not r.complete then 'incomplete' when r.average<=3 then 'parent_meeting' when r.average<6 then 'not_passed' when r.average<9 then 'passed' else 'excellent' end) order by y.start_date desc,p.start_date,c.name)
 from public.classes c join public.academic_years y on y.id=c.academic_year_id join public.grading_periods p on p.academic_year_id=y.id and p.school_id=s.school_id and public.grade_section(c.grade_level)=any(p.sections)
 left join public.category_grade_deadlines d on d.period_id=p.id and d.section=public.grade_section(c.grade_level)
 cross join lateral private.class_period_report(c.id,p.id) r
 where c.school_id=s.school_id and (exists(select 1 from public.enrollments e where e.class_id=c.id and e.student_id=s.id) or exists(select 1 from public.grades g where g.class_id=c.id and g.student_id=s.id)) and r.student_id=s.id and (s.departure_year_id is null or y.start_date<=(select start_date from public.academic_years where id=s.departure_year_id))),'[]'),
 'attendance',coalesce((select jsonb_agg(jsonb_build_object('date',a.attendance_date,'status',a.status,'check_in_at',a.check_in_at,'check_out_at',a.check_out_at,'year',y.name) order by a.attendance_date desc) from public.attendance a join public.classes c on c.id=a.class_id join public.academic_years y on y.id=c.academic_year_id where a.student_id=s.id and (s.departure_year_id is null or y.start_date<=(select start_date from public.academic_years where id=s.departure_year_id))),'[]'));
end $$;
revoke all on function private.student_report_cards(uuid) from public,anon,authenticated;

create function public.get_report_card(p_student uuid) returns jsonb language plpgsql stable security definer set search_path='' as $$
...
-- Extend the existing validated token response; no new public identity lookup.
do $$declare src text;begin
 select pg_get_functiondef('public.student_portal_overview(text)'::regprocedure) into src;
 if position('return base||jsonb_build_object' in src)=0 then raise exception 'overview source changed';end if;
 src:=replace(src,'return base||jsonb_build_object','return base||jsonb_build_object(''report'',private.student_report_cards(s.id))||jsonb_build_object');
 execute src;
end $$;
create function public.report_students() returns jsonb language plpgsql stable security definer set search_path='' as $$
declare sid uuid:=public.get_my_school_id();begin
 ...
 return coalesce((select jsonb_agg(jsonb_build_object('id',id,'first_name',first_name,'last_name',last_name,'atechos_id',atechos_id) order by last_name,first_name) from public.students where school_id=sid),'[]');
end $$;
revoke all on function public.report_students() from public,anon,authenticated;
grant execute on function public.report_students() to authenticated;
