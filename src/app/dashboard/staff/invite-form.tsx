...
export function InviteForm({ owner, parents, students }: { owner: boolean; parents: { id: string; full_name: string; email: string }[]; students: { id: string; first_name: string; last_name: string }[] }) {
  ...
  return <form className="grid gap-4 rounded-2xl border bg-white p-6 sm:grid-cols-2" onSubmit={async e => {
    ...
    <label>Full name<input className="mt-1 w-full rounded border p-2" name="full_name" required maxLength={150} /></label>
    <label>Email<input className="mt-1 w-full rounded border p-2" name="email" type="email" required maxLength={254} /></label>
    ...
    {role === 'student' && <label>Student record<select name="student_id" required className="mt-1 w-full rounded border p-2"><option value="">Choose student</option>{students.map(s => <option key={s.id} value={s.id}>{s.first_name} {s.last_name}</option>)}</select></label>}
    ...
    {url && <div className="space-y-2 rounded bg-blue-50 p-4 sm:col-span-2"><p>{delivery === 'email' ? 'Invitation email sent. You can also copy this recovery link.' : 'Invitation created. Share this private link manually.'}</p><label>Private invitation link<input readOnly value={url} className="w-full rounded border p-2" onFocus={e => e.target.select()} /></label><button type="button" onClick={async () => { try { await navigator.clipboard.writeText(url); setCopied(true) } catch { setError('Select and copy the link manually.') } }}>{copied ? 'Copied' : 'Copy link'}</button></div>}
  </form>
}
...
export function CancelInvitation({ id }: { id: string }) {
  const [error, setError] = useState(''); const [busy, setBusy] = useState(false); const router = useRouter()
  return <div><button disabled={busy} className="text-red-700" onClick={async () => { setBusy(true); try { const result = await cancelInvitation(id); setError(result.error || ''); if (!result.error) router.refresh() } catch { setError('Unable to cancel invitation.') } finally { setBusy(false) } }}>Cancel</button>{error && <p role="alert">{error}</p>}</div>
}
