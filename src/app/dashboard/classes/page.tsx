...
export default function ClassesPage() {
  ...
  async function createYear(e: FormEvent<HTMLFormElement>) {
    ...
    if (error) setError(error.message); else { setYearOpen(false); formElement.reset(); await load(); window.location.assign('/dashboard/progression') }
    setSaving(false)
  }
  ...
  return <main className="min-h-screen bg-slate-50 p-6 md:p-10"><div className="mx-auto max-w-7xl">
    <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><Link href="/onboarding" className="text-sm font-semibold text-blue-600">AtechOS</Link><h1 className="mt-1 text-3xl font-bold text-slate-900"><T text="Classes & Academic Years"/></h1><p className="mt-1 text-slate-500">Manage official grade levels, cycles, classes and school years.</p></div><div className="flex gap-2"><Link href="/dashboard/progression" className="rounded border p-2"><T text="Academic progression"/></Link><button onClick={() => setYearOpen(true)} className="rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold"><T text="+ Academic year"/></button><button onClick={() => setClassOpen(true)} className="rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white"><T text="+ Class"/></button></div></header>
    {error && <p className="mt-6 rounded-xl bg-red-50 px-4 py-3 text-sm text-red-700">{error}</p>}
    <section className="mt-8 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><h2 className="font-semibold text-slate-900"><T text="Academic years"/></h2><p className="text-sm text-slate-500">{grades.length} configured grade levels</p></div><select value={yearId} onChange={e => setYearId(e.target.value)} className="rounded-xl border border-slate-300 px-3 py-2 text-sm"><option value=""><T text="All years"/></option>{years.map(y => <option key={y.id} value={y.id}>{y.name}{y.is_current ? ' · Current' : ''}</option>)}</select></div><div className="mt-4 grid gap-3 md:grid-cols-3">{years.map(y => <div key={y.id} className="rounded-xl border border-slate-200 p-4"><p className="font-semibold">{y.name} {y.is_current && <span className="text-xs text-blue-600">CURRENT</span>}</p><p className="mt-1 text-sm text-slate-500">{y.start_date} → {y.end_date}</p></div>)}</div></section>
    <section className="mt-6 rounded-2xl border bg-white p-5"><h2 className="text-xl font-bold"><T text="Activate school sections"/></h2><p className="my-2 text-sm text-slate-600">Choose an academic year, then activate all classes for the sections your school offers. Existing records are preserved. Primaire groups AF1–AF6; these also belong to fundamental education.</p><div className="grid gap-3 md:grid-cols-4">{schoolSections.map(section=>{const rows=visible.filter(c=>gradeSection(grades.find(g=>g.id===c.grade_level_id)?.code)===section.code); const expected=grades.filter(g=>gradeSection(g.code)===section.code); const active=expected.length>0 && expected.every(g=>rows.some(c=>c.grade_level_id===g.id && c.enabled)) && rows.every(c=>c.enabled); return <div key={section.code} className="rounded border p-3"><h3 className="font-semibold">{section.name}</h3><p>{section.grades}</p><button disabled={!yearId || saving} onClick={()=>toggleSection(section.code,!active)} className="mt-3 rounded border p-2 disabled:opacity-50">{active ? 'Deactivate' : 'Activate classes'}</button></div>})}</div></section>
    <section className="mt-8 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-xl font-bold text-slate-900"><T text="Grade structure"/></h2><p className="mt-1 text-sm text-slate-500">Préscolaire → Fondamental (3 cycles) → Nouveau Secondaire.</p><div className="mt-5 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{grouped.map(([code, label]) => <div key={code} className="rounded-xl border border-slate-200 p-4"><p className="font-semibold text-slate-900">{label}</p><div className="mt-3 flex flex-wrap gap-2">{grades.filter(g => g.cycle_code === code).map(g => <span key={g.id} title={g.name} className="rounded-lg bg-slate-100 px-3 py-1.5 text-sm font-medium">{g.short_name}</span>)}</div></div>)}</div></section>
    <section className="mt-6"><div className="mb-3 flex items-center justify-between"><h2 className="text-xl font-bold text-slate-900"><T text="Classes"/></h2><span className="text-sm text-slate-500">{visible.length} class{visible.length === 1 ? '' : 'es'}</span></div>{loading ? <p className="text-slate-500"><T text="Loading..."/></p> : <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{visible.map(c => <article key={c.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start justify-between"><h3 className="text-lg font-semibold">{c.name}</h3><span className="rounded-full bg-slate-100 px-2 py-1 text-xs">{c.enabled ? 'Active' : 'Inactive'} · {c.room || 'No room'}</span></div><p className="mt-2 text-sm text-slate-500">{grades.find(g => g.id === c.grade_level_id)?.name || 'Grade level not set'}</p><p className="mt-1 text-xs font-semibold text-blue-600">{grades.find(g => g.id === c.grade_level_id)?.short_name || ''}</p><p className="mt-3 text-xs font-medium text-slate-500">{years.find(y => y.id === c.academic_year_id)?.name || 'Academic year'}</p></article>)}</div>}</section>
