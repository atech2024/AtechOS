...
export default async function StudentPortal() {
 ...
 return <main className="mx-auto max-w-5xl p-6"><header className="flex justify-between print:hidden"><h1 className="text-2xl font-bold"><T text="Student portal"/></h1><form action={studentLogout}><button className="rounded border p-3"><T text="Logout"/></button></form></header><FamilyBulletins studentMode data={{students:[data.student],grades}} /><section className="print:hidden"><h2 className="text-2xl font-bold"><T text="Assignments"/></h2>{data.assignments?.map((a:{id:string;title:string;description:string;due_at:string;attachment_url:string|null},i:number)=><article key={i} className="my-3 rounded border p-4"><h3 className="font-semibold">{a.title}</h3><p className="whitespace-pre-wrap">{a.description}</p><p>{a.due_at?new Date(a.due_at).toLocaleDateString():''}</p>{a.attachment_url && <a href={`/student/attachment/${a.id}`} target="_blank" rel="noopener noreferrer" className="text-blue-700 underline"><T text="Open attachment ↗"/></a>}</article>)}{!data.assignments?.length && <p><T text="No current assignments."/></p>}</section></main>
}
