...
export default function ClassesPage() {
  ...
  return <main className="min-h-screen bg-slate-50 p-6 md:p-10"><div className="mx-auto max-w-7xl">
    ...
    <section className="mt-6 rounded-2xl border bg-white p-5"><h2 className="text-xl font-bold">Activate school sections</h2><p className="my-2 text-sm text-slate-600">Choose an academic year, then activate all classes for the sections your school offers. Existing records are preserved. Primaire groups AF1–AF6; these also belong to fundamental education.</p><div className="grid gap-3 md:grid-cols-4">{schoolSections.map(section=>{const rows=visible.filter(c=>gradeSection(grades.find(g=>g.id===c.grade_level_id)?.code)===section.code); const active=rows.length>0 && rows.every(c=>c.enabled); return <div key={section.code} className="rounded border p-3"><h3 className="font-semibold">{section.name}</h3><p>{section.grades}</p><button disabled={!yearId || saving} onClick={()=>toggleSection(section.code,!active)} className="mt-3 rounded border p-2 disabled:opacity-50">{active ? 'Deactivate' : 'Activate classes'}</button></div>})}</div></section>
