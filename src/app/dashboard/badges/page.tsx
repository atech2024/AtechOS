...
export default function BadgesPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const yearResult = academicYearIds.length ? await getSupabase().from('academic_years').select('id,name,start_date').in('id', academicYearIds) : {data:[],error:null}
  ...
  return <main className="min-h-screen bg-slate-50 p-6 md:p-10">
    <div className="mx-auto max-w-6xl print:hidden">
      ...
      <section className="mt-8 grid gap-6 md:grid-cols-[320px_1fr]">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><h2 className="font-semibold text-slate-900"><T text="Badge preview"/></h2><p className="mt-1 text-sm text-slate-500">Only school-facing identification data appears on the printed badge.</p><div className="mt-6 flex justify-center"><BadgeCard school={school} student={selected} badge={badge} enrollment={enrollment} photoSrc={photoSrc}/></div>{selected && !photoSrc && <p className="mt-5 rounded-xl bg-amber-50 px-4 py-3 text-sm text-amber-800">No student photo is available. Upload one before printing the badge.</p>}</div>
      </section>
    </div>
