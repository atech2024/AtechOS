...
type Enrollment = { student_id: string; class_id: string; classes: { name: string; academic_years: { name: string } | null } | null }
  ...
  useEffect(() => {
    async function load() {
      ...
      const [sr, br, er, sch] = await Promise.all([
        ...
        supabase.from('enrollments').select('student_id,class_id,classes(name,academic_years(name))').eq('school_id', schoolId).eq('status', 'active'),
        supabase.from('schools').select('name,logo_url').eq('id', schoolId).single(),
      ])
      const firstError = sr.error || br.error || er.error || sch.error
      if (firstError) setError(firstError.message)
      setStudents((sr.data || []) as Student[]); setBadges((br.data || []) as Badge[]); setEnrollments((er.data || []) as unknown as Enrollment[]); setSchool((sch.data || null) as School | null)
...
function BadgeCard({ school, student, badge, enrollment }: { school: School | null; student: Student | null; badge: Badge | null; enrollment: Enrollment | null }) {
  return <div className="badge-card relative h-[54mm] w-[85.6mm] overflow-hidden rounded-[4mm] border border-slate-300 bg-white shadow-lg print:rounded-none print:border-0 print:shadow-none">
    ...
    <div className="flex gap-3 px-4 py-3"><div className="h-[22mm] w-[18mm] shrink-0 overflow-hidden rounded border border-slate-200 bg-slate-100">{student?.photo_url ? <img src={student.photo_url} alt="Student" className="h-full w-full object-cover"/> : <div className="flex h-full items-center justify-center text-[8px] text-slate-400">PHOTO</div>}</div><div className="min-w-0 flex-1"><p className="text-[13px] font-black uppercase leading-tight text-slate-900">{student ? `${student.first_name} ${student.last_name}` : 'Student name'}</p><p className="mt-1 text-[10px] font-bold text-blue-700">{enrollment?.classes?.name || 'Class'}</p><p className="mt-1 text-[8px] text-slate-500">Année scolaire</p><p className="text-[10px] font-semibold text-slate-800">{enrollment?.classes?.academic_years?.name || '—'}</p></div></div>
