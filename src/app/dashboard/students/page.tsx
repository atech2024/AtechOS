...
type Student = {
  id: string; atechos_id: string; nis: string | null; student_code: string; first_name: string; last_name: string
...
export default function StudentsPage() {
  ...
  const [loading, setLoading] = useState(true); const [saving, setSaving] = useState(false); const [error, setError] = useState(''); const [showForm, setShowForm] = useState(false); const [editing, setEditing] = useState<Student | null>(null); const [form, setForm] = useState(emptyForm)

  async function loadStudents() {
    setLoading(true); setError('')
    const [sr, br] = await Promise.all([
      supabase.from('students').select('id,atechos_id,nis,student_code,first_name,last_name,date_of_birth,sex,phone,email,address,emergency_contact_name,emergency_contact_phone,active').order('last_name').order('first_name'),
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (result.error) { setError(result.error.message.replace('student_code_already_exists','Student code already exists').replace('nis_already_exists','NISU already exists').replace('first_name_required','First name is required').replace('last_name_required','Last name is required')); setSaving(false); return }
    ...
    if (form.badge_uid.trim()) {
      ...
      if (badgeResult.error) { setError(badgeResult.error.message.replace('badge_already_assigned','This badge is already assigned to another student').replace('badge_uid_required','Badge UID is required')); setSaving(false); return }
    }
