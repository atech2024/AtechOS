        ...
        <section className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {modules.map(([title, description, href]) => (
            <article key={title} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              {href === '#' ? (
                <span className="mt-5 inline-block text-sm font-semibold text-slate-400">Coming next</span>
              ) : (
                <Link href={href} className="mt-5 inline-block text-sm font-semibold text-blue-600 hover:underline">Open module →</Link>
              )}
            </article>
          ))}
        </section>
