...
begin
 begin
  ...
  perform set_config('request.jwt.claim.sub',owner_a::text,true);
  set local role authenticated;
  spare:=public.activate_grading_period(yr,'Primary archive','PA','2026-04-01','2026-05-31',array['primary']);
  payload:=public.get_report_card(child1);
  if exists(select 1 from jsonb_array_elements(payload->'cards') c where c->>'class_id'=other_cls::text) then raise exception 'TEST empty transferred class included';end if;
  perform public.assign_subject_to_class(other_cls,subject,null);
  perform public.save_student_record(jsonb_build_object('first_name','Corrected','last_name','One','class_id',other_cls),child1);
  perform public.create_grade(child1,subject,other_cls,'Archived',7,10,null,spare);
  perform public.publish_reviewed_grades(array(select id from public.grades where student_id=child1 and class_id=other_cls));
  perform public.save_student_record(jsonb_build_object('first_name','Corrected','last_name','One','class_id',cls),child1);
  payload:=public.get_report_card(child1);
  if not exists(select 1 from jsonb_array_elements(payload->'cards') c where c->>'class_id'=other_cls::text and (c->>'average')::numeric=7) then raise exception 'TEST transferred class archive lost';end if;
  reset role;
