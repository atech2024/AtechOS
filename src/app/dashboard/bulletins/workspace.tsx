...
export default function BulletinsPage() {
  ...
  return <main className="min-h-screen bg-slate-50 p-6 md:p-10 print:bg-white print:p-0"><div className="mx-auto max-w-6xl">
    <header className="mb-8 print:hidden"><Link href="/onboarding" className="text-sm font-semibold text-blue-600">AtechOS</Link><h1 className="mt-1 text-3xl font-bold text-slate-900"><T text="Bulletins"/></h1><p className="mt-1 text-slate-500">Generate a student bulletin with grading-period averages, final averages and school passing threshold.</p></header>
    ...
    <section className="mb-6 grid gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm md:grid-cols-2 print:hidden">
      <label className="text-sm font-medium"><T text="Class"/><select value={classId} onChange={e => setClassId(e.target.value)} className="mt-2 w-full rounded-xl border border-slate-300 px-3 py-3"><option value=""><T text="Select class..."/></option>{classes.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></label>
      <label className="text-sm font-medium"><T text="Student"/><select value={studentId} onChange={e => setStudentId(e.target.value)} disabled={!classId} className="mt-2 w-full rounded-xl border border-slate-300 px-3 py-3"><option value=""><T text="Select student..."/></option>{students.map(s => <option key={s.id} value={s.id}>{s.first_name} {s.last_name}{s.student_code ? ` · ${s.student_code}` : ''}</option>)}</select></label>
      <div className="md:col-span-2"><button type="button" onClick={() => window.print()} disabled={!studentId} className="rounded-xl bg-blue-600 px-6 py-3 font-semibold text-white disabled:opacity-50"><T text="Print / Save PDF"/></button></div>
    </section>
