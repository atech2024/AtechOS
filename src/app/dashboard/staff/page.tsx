...
export default async function StaffPage() {
  ...
  return <main className="mx-auto max-w-6xl space-y-6 p-6"><Link href="/dashboard">← Dashboard</Link><h1 className="text-3xl font-bold">Users & staff</h1><InviteForm owner={context.owner} parents={parents.data || []} students={students.data || []} />
    <section className="overflow-x-auto rounded-2xl border bg-white p-6"><h2 className="mb-4 text-xl font-semibold">School access</h2><table className="w-full text-left"><thead><tr><th>Name / email</th><th>Role</th><th>Status</th><th>Action</th></tr></thead><tbody>{members.data?.map(m => {
