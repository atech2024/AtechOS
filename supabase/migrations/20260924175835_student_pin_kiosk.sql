-- Attendance writes require the student's PIN, never a staff role.
drop policy if exists attendance_write on public.attendance;
revoke insert,update,delete,truncate on public.attendance from public,anon,authenticated;
revoke all on function public.scan_student_code(text,uuid), public.scan_student_badge(text,uuid,timestamptz,integer), public.record_attendance(uuid,uuid,date,text,timestamptz,timestamptz,integer,text), public.update_attendance_checkout(uuid,timestamptz) from public,anon,authenticated;
...
declare s public.students; cred private.student_credentials; cl public.classes; a public.attendance;
 ts timestamptz:=now(); d date:=(now() at time zone 'America/Port-au-Prince')::date; result text; class_count integer;
begin
 ...
 if a.id is null then
  ...
  values(s.school_id,s.id,cl.id,d,'present',ts,0,s.user_id) returning * into a; result:='check_in';
 elsif a.check_in_at is null then
  update public.attendance set check_in_at=ts,status='present',recorded_by=s.user_id,updated_at=ts where id=a.id returning * into a; result:='check_in';
 elsif ts-a.check_in_at<interval '60 seconds' then result:='duplicate_scan';
 ...
 else result:='already_complete'; end if;
